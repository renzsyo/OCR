...
CLASSIFIER_PATH = os.path.join(os.path.dirname(__file__), "efficientnet_b4_best.pth")
...
def load_classifier() -> None:
    ...
    try:
        ...
        if isinstance(checkpoint, dict):
            ...
            model.classifier[1] = nn.Linear(model.classifier[1].in_features, len(CLASSIFIER_LABELS))
...
def classify_id_type(image: np.ndarray) -> tuple[str | None, float]:
    ...
    is unavailable or confidence is below the threshold.
    """
...
def decode_qr_pyzbar(image: np.ndarray) -> str | None:
    ...
    try:
        image = _safe_resize(image, max_w=800)
        rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        pil_img = Image.fromarray(rgb)
