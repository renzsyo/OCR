...
class InferenceHandler:
    ...
    def _check_inference_done(self):
        p = self.parent
    ...
    def run_inference_national_id(self, front_image, back_image):
        p = self.parent
        debug = getattr(p, "debug_mode", False)
        ...
        # Step 2: Scan front (OCR)
        front_result = scan_national_id_front(front_image)
    ...
    def infer_only_driver_license_upload(self):
        p = self.parent
        if not p.front_file:
            return
    ...
    def infer_only_driver_license_camera(self):
        p = self.parent
    ...
    def infer_only_national_id_camera(self):
        p = self.parent
    ...
    def infer_only_national_id_upload(self):
        p = self.parent
    ...
    def validate_passport_result_sync(self, result):
        p = self.parent
        try:
    ...
    def validate_driver_license_result_sync(self, result):
        p = self.parent
        try:
            data = result.get("parsed", {}).get("Driverslicense/OCR", {})
    ...
    def validate_national_id_result_sync(self, result):
        p = self.parent
        try:
    ...
    def run_inference_national_id(self, front_image, back_image):
        ...
        # Step 2: Scan front (OCR) with debug
        ...
        # ✅ Store front debug image (back has no OCR so no bounding boxes)
    ...
    def match_national_id(self, qr_result, front_result):
        mismatches = []
        try:
            ...
            if not front_fields:
                ...
                # ✅ Fail if PCN is missing from front
