"""
=============================================================
  Philippine ID Classifier - EfficientNet-B4 Inference + Heatmap
  Classes: Passport vs National ID (PhilID)
=============================================================

HOW TO USE:
  Single image test:
    python inference_efficientnet.py --image "path/to/image.jpg"

  Folder test:
    python inference_efficientnet.py --folder "path/to/folder"

  Live camera:
    python inference_efficientnet.py --camera

  Save without displaying:
    python inference_efficientnet.py --image "test.jpg" --no-show

  Skip heatmap, just inference:
    python inference_efficientnet.py --image "test.jpg" --no-gradcam
=============================================================
"""
...
#  CONFIG
# ─────────────────────────────────────────────
MODEL_PATH           = r"C:\Users\Renzo\Documents\MindVision\models\efficientnet_b4_v2\efficientnet_b4_best.pth"
OUTPUT_DIR           = r"C:\Users\Renzo\Documents\MindVision\efficientnet_results"
...
class GradCAM:
    """
    Grad-CAM for EfficientNet-B4.
    Hooks into the last convolutional block (features[8])
    which gives the most semantically rich feature maps.
    """
    def __init__(self, model):
        ...
        # EfficientNet-B4: hook into last conv block (features[8])
...
def create_visualization(original_img, overlay, heatmap_colored,
    ...
    orig_bgr      = cv2.cvtColor(np.array(original_img.convert("RGB")), cv2.COLOR_RGB2BGR)
    orig_panel    = add_label(resize(orig_bgr), "Original")
    heatmap_panel = add_label(resize(heatmap_colored), "Attention Map")
    overlay_panel = add_label(resize(overlay), f"{class_name.upper()} ({confidence:.1%})")
    ...
    cv2.putText(title_bar,
                f"EfficientNet-B4 Grad-CAM | {class_name.upper()} | {confidence:.1%} | {filename}",
...
def process_image(model, image_path, show=True, use_gradcam=True):
    ...
    # Standard inference
    class_name, confidence, elapsed, probs = predict(model, original_img)
    ...
    if use_gradcam:
        try:
            ...
            heatmap, class_idx, _ = gradcam.generate(input_tensor)
            ...
            if heatmap is not None:
                ...
                save_name = f"effnet_gradcam_{os.path.splitext(filename)[0]}.jpg"
                save_path = os.path.join(OUTPUT_DIR, save_name)
                cv2.imwrite(save_path, visualization)
                print(f"  🗺️  Heatmap saved → {save_path}")

                if show:
                    cv2.imshow(f"EfficientNet-B4 Grad-CAM: {filename}", visualization)
                    print("  Press any key to continue...")
                    cv2.waitKey(0)
                    cv2.destroyAllWindows()
            else:
                print("  ⚠️  Grad-CAM returned empty — saving plain result")
                _save_plain(original_img, class_name, confidence, filename, show)
...
def _save_plain(original_img, class_name, confidence, filename, show):
    ...
    save_path = os.path.join(OUTPUT_DIR, f"effnet_{os.path.splitext(filename)[0]}.jpg")
    cv2.imwrite(save_path, img_bgr)
    print(f"  💾 Saved → {save_path}")
    if show:
        cv2.imshow(f"EfficientNet-B4: {filename}", img_bgr)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def test_camera(model, camera_index=0):
    ...
    while True:
        ...
        cv2.imshow("EfficientNet-B4 ID Classifier", display)
        key = cv2.waitKey(1) & 0xFF

        if key == ord("q"):
            break
        elif key == ord(" "):
            save_path = f"effnet_capture_{int(time.time())}.jpg"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="EfficientNet-B4 ID Classifier - Inference + Grad-CAM"
    )
