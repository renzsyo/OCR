...
def draw_bounding_boxes(image, ocr_results):
    ...
    return debug_img
def extract_national_id_front_fields(rec_texts, rec_scores):
...
# ====================================================
def scan_national_id_front(image):
    if isinstance(image, str):
        image = cv2.imread(image)
    if image is None:
        return {"parsed": {"NationalID/Front": None, "valid": False}, "raw": None}
    ...
    fields = extract_national_id_front_fields(rec_texts, rec_scores)
    result["NationalID/Front"] = fields
    result["valid"] = len(fields) > 0
